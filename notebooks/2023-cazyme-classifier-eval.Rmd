...
fam.tax.df <- rbind(cazy_family_long_df, fam.euk.gh.subset)
...
tax_grp <- rep('All', nrow(cazy_family_long_df))
tax_grp <- append(tax_grp, rep('Eukaryote', ((nrow(fam.tax.df) - nrow(cazy_family_long_df)))))
euk.count <- nrow(fam.tax.df) - nrow(cazy_family_long_df)
...
tax_grp <- append(tax_grp, rep('Bacteria', ((nrow(fam.tax.df) - nrow(cazy_family_long_df) - euk.count))))
fam.tax.df$Tax_group <- tax_grp
...
p.fam.tax.f1.ci.per.cl <- build.class.tax.per.class.p(f1.tax.fam.sum.df, "F1-score")
...
# save plots
pdf(file = "../report/cazy_family_classification/tax_performance/per_cazy_class/f1.fam.tax.ci.pdf",  width = 11, height = 8)
p.fam.tax.f1.ci.per.cl
dev.off()
